...
const PostCardBox = (props) => {
  ...
  return (
    <PostDefaultModal PostDefaultTitle={"우표 선택"} cancelClick={props.cancelClick}>
      <div className={styles.postCardContainer}>
        {cards.map((card, idx) => (
          <img
            key={card.src}
            className={selectedCard?.src === card.src ? `${styles.postCard} ${styles.selected}` : styles.postCard}
